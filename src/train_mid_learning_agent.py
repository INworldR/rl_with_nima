...
for _ in range(nr_trainings):
    ...
    print(
        "total reward:",
        agent.evaluate()["evaluation"]["sampler_results"]["episode_reward_mean"],
    )

# save the agent model to disk
# agent.save("trained_agent_cartpole")

checkpoint_dir = "./cartpole_dqn_hi_checkpoints"
checkpoint_path = agent.save(checkpoint_dir)
print(f"Checkpoint saved at: {checkpoint_path}")
# Load the saved agent from the checkpoint
# agent.restore(checkpoint_path)
# print(f"Agent restored from: {checkpoint_path}")
